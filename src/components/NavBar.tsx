...
const NavBar: React.FC<NavBarProps> = ({
  ...
  if (isMobile && isSettingsPage) {
    return <nav className="fixed top-0 left-0 right-0 z-50 w-full border-b bg-background/95 backdrop-blur-lg shadow-sm">
        <div className="flex h-16 items-center justify-between py-3 px-4">
          {/* Left section - Sidebar Icon + Logo */}
          <div className="flex items-center gap-1">
            <Button onClick={handleMobileMenuClick} variant="ghost" size="icon" className={`h-10 w-10 p-0 hover:bg-muted/50 transition-all duration-200 rounded-md md:hidden ${theme === 'light' ? 'text-black' : 'text-white'}`}>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="text-muted-foreground hover:text-foreground hover:bg-muted transition-all duration-200">
  ...
  return <>
      <nav className="sticky top-0 z-40 w-full border-b bg-background/90 backdrop-blur-lg shadow-sm">
        <div className="w-full flex h-16 items-center justify-between py-3 px-4 sm:px-6 lg:px-8 xl:px-12">
          ...
          <div className="flex items-center gap-1">
            ...
            <Sheet open={mainMenuOpen} onOpenChange={setMainMenuOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className={`h-10 w-10 p-0 hover:bg-muted/50 transition-all duration-200 rounded-md md:hidden ${theme === 'light' ? 'text-black' : 'text-white'}`}>
                  <Menu className="h-6 w-6" />
                </Button>
              </SheetTrigger>
              <SheetContent side="left" className="w-64 p-0">
                <div className="p-6">
                  ...
                  <nav className="space-y-2">
                    {mainMenuItems.map(item => <Button key={item.id} onClick={() => handleMainMenuNavigation(item.path)} variant="ghost" className={`w-full justify-start text-left p-3 h-auto ${location.pathname === item.path ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-muted'}`}>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="start" className="w-48 bg-background border border-border rounded-lg shadow-lg p-1">
          ...
          {isMobile && !isSettingsPage && <div className="flex items-center gap-2">
              {/* Theme Toggle */}
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
          ...
          {!isMobile && <div className="flex items-center gap-3">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    {showSearch ? <div className="relative animate-fade-in w-full max-w-[280px] md:max-w-none">
                        <Input ref={searchInputRef} className="w-full md:w-[240px] pr-8 md:w-[300px] shadow-sm h-12 focus:outline-none focus:ring-0 focus-visible:ring-0 focus-visible:ring-offset-0" placeholder="Search bookmarks..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} autoFocus onBlur={e => {
                    ...
                    if (!e.relatedTarget || !e.relatedTarget.closest('.search-container')) {
                      ...
                      </div> : <Button variant="ghost" size="icon" onClick={() => setShowSearch(true)} className="text-muted-foreground hover:text-foreground hover:bg-muted transition-all duration-200">
              ...
              {(!showSearch || window.innerWidth > 768) && <>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button onClick={() => setAddDialogOpen(true)} className="flex items-center gap-1 gradient-primary hover:opacity-95 transition-all duration-200 hover:shadow-md px-5 py-6">
      ...
      {isMobile && !isSettingsPage && <div className="fixed bottom-0 left-0 right-0 z-50 bg-background border-t flex justify-around items-center h-16">
          {/* Handle custom bottom bar or default behavior */}
          {customBottomBar ?
      // Custom bottom bar for specific pages (like Archive in selection mode)
      <>
              {/* Left button */}
              {customBottomBar.leftButton && <Button variant="ghost" className="h-full w-1/3 flex flex-col items-center justify-center rounded-none gap-1 hover:bg-muted transition-colors duration-200" onClick={customBottomBar.leftButton.onClick} disabled={customBottomBar.leftButton.disabled}>
              ...
              {/* Center button */}
              {customBottomBar.centerButton && <Button variant="ghost" className="h-full w-1/3 flex flex-col items-center justify-center rounded-none gap-1 hover:bg-muted transition-colors duration-200" onClick={customBottomBar.centerButton.onClick} disabled={customBottomBar.centerButton.disabled}>
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button variant="ghost" className="h-full w-1/3 flex items-center justify-center rounded-none my-0 px-0 mx-[5px] font-extralight text-slate-50 text-lg hover:bg-muted transition-colors duration-200">
                    <MoreHorizontal className="h-8 w-8" />
                  </Button>
                </SheetTrigger>
                <SheetContent side="bottom" className="h-auto pb-16 rounded-t-xl">
                  <div className="grid gap-4 py-4">
                    <Button variant="ghost" className="flex justify-start" onClick={goToSettings}>
                      <Settings className="mr-2 h-5 w-5" />
                      Settings
                    </Button>
                    <Button variant="ghost" className="flex justify-start" onClick={handleSignOut}>
      ...
      <>
              ...
              {!isSelectMode ? <Drawer open={addDialogOpen} onOpenChange={setAddDialogOpen}>
                  ...
                  <DrawerContent className="p-4">
                    <AddBookmarkForm open={addDialogOpen} onOpenChange={setAddDialogOpen} onSubmit={onAddBookmark} drawerMode={true} />
                  </DrawerContent>
                </Drawer> :
        // In select mode, show Archive button only on bookmark and trash pages, hide on archive page
        location.pathname !== '/archive' && <Button variant="ghost" className="h-full w-1/3 flex flex-col items-center justify-center rounded-none gap-1 hover:bg-muted transition-colors duration-200" onClick={handleBottomBarArchive} disabled={selectedBookmarks.length === 0}>
                  ...
                  <DrawerContent className="p-4">
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        <Input ref={mobileSearchInputRef} className="w-full shadow-sm pr-8 focus:outline-none focus:ring-0 focus-visible:ring-0 focus-visible:ring-offset-0" placeholder="Search bookmarks..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} autoFocus />
                        {searchQuery && <button className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground" onClick={handleMobileClearSearch}>
                            <X className="h-4 w-4" />
                          </button>}
                      </div>
                    </div>
        ...
        location.pathname !== '/archive' && <Button variant="ghost" className="h-full w-1/3 flex flex-col items-center justify-center rounded-none gap-1 hover:bg-muted transition-colors duration-200" onClick={handleBottomBarDelete} disabled={selectedBookmarks.length === 0}>
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button variant="ghost" className="h-full w-1/3 flex items-center justify-center rounded-none hover:bg-muted transition-colors duration-200">
